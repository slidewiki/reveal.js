...
var RevealToolbar = window.RevealToolbar || (function(){
	...
	function loadPlugin() {
		...
		if (!head.browser.ie || head.browser.version >= 9) {
			...
			var deckUrl = option(options.deckUrl, false);
            var contactUsUrl = option(options.contactUsUrl, false);
			...
			if (deckUrl) {
                dom.deckButton = createToolbarButton(
                    'fa-th-large',
                    function() { window.open(deckUrl, '_blank'); });
            }

            if (contactUsUrl) {
                dom.contactButton = createToolbarButton(
                    'fa-envelope',
                    function() { window.open(contactUsUrl, '_blank'); });
            }
